...
class AuthMiddleware {
  ...
  public static isAuthenticated = catchAsyncErrors(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!token) {
          return res
            .status(401)
            .json({ message: "Unauthorized: No token provided", unauthorized: true });
        }
        ...
        if (!user) {
          return res
            .status(401)
            .json({ message: "Unauthorized: User not found", unauthorized: true });
        }
  ...
  public static streamAuth = catchAsyncErrors(
    async (req: Request, res: Response, next: NextFunction) => {
      try {
