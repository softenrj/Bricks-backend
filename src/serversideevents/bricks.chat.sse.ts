...
export class BRICKSCHATSSE_SERVICE {
    ...
    public static async getChatObject(chatId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId, projectId: string, prompt: string): Promise<IBricksChat | null> {
        try {
            let chat = chatId
    ...
    public static async processChat(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            //? Load history
            const history = await bricks_message.find({ chatId: chat._id })
                .sort({ createdAt: -1 })
                .limit(HISTORY_LIMIT)
                .lean();
