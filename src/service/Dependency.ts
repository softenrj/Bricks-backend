...
export const getDependencyList = async (projectId: string, userId: mongoose.Types.ObjectId): Promise<DependencyInfo[]> => {
    ...
    const detailedDeps = await Promise.all(
        depNames.map(async (name) => {
            try {
                ...
                const { data } = await axios.get(`https://registry.npmjs.org/${encodeURIComponent(name)}/latest`);
