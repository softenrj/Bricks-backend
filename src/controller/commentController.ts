...
export const newComment = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const commentObject: Record<string, any> = { content: content, type: type, userId: userId, typeId: typeId, profile: profile };
        if (parentId) {
...
export const getComment = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const query: Record<string, any> = { type: type, typeId: new mongoose.Types.ObjectId(typeId as any) };
        ...
        sendResponse(res, 200, { success: true, message: "fetched message", data: data, nextCursor: hasNextPage ? data[data.length - 1].createdAt : null })
