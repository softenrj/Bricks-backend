...
export const accountChange = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        sendResponse(res, 200, { success: true, message: "Profile updated successfully", data: updatedProfile });
    } catch (error) {
...
export const getUserStats = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const statsAgg = await UserStats.aggregate([
            { $match: { userId } },
            { $lookup: { from: "achievements", localField: "achievements", foreignField: "_id", as: "achievements" } },
        ...
        sendResponse(res, 200, { success: true, message: "successfully fetched userStats", data: userStats })
    } catch (error) {
        console.error("Error getting User data:", error);
        sendResponse(res, 500, { success: false, message: "Internal Server Error" });
    }
