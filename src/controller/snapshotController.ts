...
export const extendSnapLife = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (
      !snapv1Id ||
      !snapv2Id ||
      !mongoose.isValidObjectId(snapv1Id) ||
      !mongoose.isValidObjectId(snapv2Id)
    ) {
  ...
  try {
    ...
    if (
      ...
      sendResponse(res, 400, {
        success: false,
        message: "Valid SnapSort IDs (v1 and v2) are required.",
      });
      return;
    }
    ...
    sendResponse(res, 200, {
      success: true,
      message: "Files synced and snapshots cleaned up successfully.",
    });
  } catch (error) {
...
export const rollbackSnap = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    sendResponse(res, 200, {
      success: true,
      message: "Files rollback and snapshots cleaned up successfully.",
      data: files,
    });
  } catch (error) {
    console.error("Error inside commitSnap:", error);
