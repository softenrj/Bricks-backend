...
export class ProjectContextController {
    ...
    public static getProject = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!project) {
                sendResponse(res, 400, { success: false, message: "Project Not Found ! Make sure Id is Correct" });
                return;
            }

            const childFiles: IProjectFile[] = (await ProjectFile.find({ projectId, userId }).lean()) as unknown as IProjectFile[];
    ...
    public static getProjectChild = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            sendResponse(res, 200, { success: true, message: "Children fetched successfully", data: childrens });
        } catch (error) {
